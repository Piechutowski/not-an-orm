...
- **D41 — One binary, named `nao`; the language names the artifacts**
  (2026-07-12): the tool and the language have different names on purpose.
  The binary is `nao` ("not an ORM" — the toolkit, not a DBML parser), a
  single CLI with the language server as its `lsp` subcommand (the pattern
  of `deno lsp` / `gleam lsp` / `ruff server`). The language is EDBML and
  names every language artifact: the `edbml/` package tree, `.edbml`
  schema files, the `schema.edbml` input convention (D39) and the
  generated `edbml_models.go` / `edbml_queries.go` / `edbml_schema.sql`
  (provenance from the schema — stable even if the tool is ever renamed
  again). Generated headers cite the command: "Code generated by nao gen".
  Plain `.dbml` files remain fully supported as inputs — EDBML is a
  superset.
